    ...
    <div class="section-body">
      <div class="card">
        <div class="card-header">
          <h4>Data Surat Keluar Yang Dibatalkan</h4>
        </div>
              ...
              foreach ($row->result() as $key => $data) { ?>
                <tr>
                  ...
                  <td class="text-center" width="130px" style="width:18%;">
                    <form action="<?= site_url('Suratkeluar/kembalikan') ?>" method="post">
                      <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
                      &nbsp;
                      <input type="hidden" name="kode_sk" value="<?= $data->kode_sk ?>">
                      <button onclick="return confirm('Apakah Anda Yakin Ingin Mengembalikan ?')" class="btn btn-success btn-xs">
                        <i class="fa fa-recycle"></i> Kembalikan
                      </button>
                    </form>
                  </td>
                </tr>
